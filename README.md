...
3. 起動時に `php artisan migrate --force` を実行してから `php artisan config:cache` と `php artisan serve` を実行し、`PORT` を使って起動されます（`upload_max_filesize=50M`, `post_max_size=200M`, `memory_limit=512M` を指定）。
